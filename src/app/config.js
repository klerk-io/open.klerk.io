module.exports = {
  ...
  headers: {
    access: {
      headers:
        process.env.HEADERS_ACCESS_CONTROL_ALLOW_HEADERS || "Content-Type",
      methods:
        process.env.HEADERS_ACCESS_CONTROL_ALLOW_METHODS ||
        "OPTIONS, POST, GET, PUT, DELETE",
      origin: process.env.HEADERS_ACCESS_CONTROL_ALLOW_ORIGIN || "*",
      expose:
        process.env.HEADERS_ACCESS_CONTROL_EXPOSE_HEADERS || "Content-Length"
    },
    security: {
      sts:
        process.env.HEADERS_STRICT_TRANSPORT_SECURITY ||
      ...
      ref:
        process.env.HEADERS_REFERRER_POLICY || "strict-origin-when-cross-origin"
    }
  },
  lockers: {
    ...
    entity: {
      signature: {
        ...
        data: null, // user data
        requests: null, // remaining requests
      ...
      indexes: ["createdAt", "expiresAt", "version"],
      defaults: {
  ...
  logs: {
    table: {
      name: process.env.LOGS_TABLE_NAME || "Logs"
    },
    entity: {
      signature: {
        id: null,
